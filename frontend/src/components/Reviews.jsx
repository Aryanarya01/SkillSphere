        ...
        {reviews.map((review) => (
          <div key={review._id} className="bg-white rounded-2xl shadow-md p-6">
            ...
            <div className="flex items-center gap-4">
              <img
                src={`http://localhost:9090${review.reviewer?.profilePicture}`}
                alt="reviewer"
                className="w-14 h-14 rounded-full object-cover"
              />

              <div>
                <h3 className="font-bold text-lg">{review.reviewer?.name}</h3>

                <p className="text-gray-500 text-sm">{review.job?.title}</p>
              </div>
            </div>
